...
- `internal/loader` discovers and loads Skel source files. `internal/parser` owns strict syntax parsing and its `grammar` subpackage, which contains the Participle grammar and syntax-tree representation. `internal/analyzer` builds and validates public semantic model objects from that syntax tree, while `internal/hasher` derives compatibility hashes. `internal/compiler` coordinates loading, recovery, diagnostics, import resolution, semantic analysis, hashes, and incremental workspace analysis. The public `model` package contains only parser-independent semantic data and source positions.
- `internal/codegen/{golang,skeleton,typescript}` own generated Go, Skel, and TypeScript output. `internal/codegen/common` provides public-contract projection, rendering, managed-output, and generated-text helpers without depending on a target generator.
