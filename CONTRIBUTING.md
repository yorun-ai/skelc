...
The generator identifies owned files by the `Code generated by skelc. DO NOT EDIT.` marker near the top of each output. Use temporary directories in manual tests and verify the target before running a generation command against an existing directory, because current generated paths are overwritten even when unrelated unmarked files are preserved.
