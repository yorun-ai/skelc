...
func (g *_Gen) buildResourceSchema(resource *model.Resource) *_ResourceSchema {
	schema := &_ResourceSchema{
		...
		Description: resource.Description, Deprecated: resource.Deprecated,
		DeprecatedReason: resource.DeprecatedReason, Checks: g.buildResourceCheckSchemas(resource.Checks),
	...
	for _, action := range resource.Actions {
		schema.Actions = append(schema.Actions, &_ResourceActionSchema{
			...
			DeprecatedReason: action.DeprecatedReason, Checks: g.buildResourceCheckSchemas(action.Checks),
		})
	}
...
func (g *_Gen) buildResourceCheckSchemas(checks []*model.ResourceCheck) []*_ResourceCheckSchema {
	...
	for _, check := range checks {
		schemas = append(schemas, &_ResourceCheckSchema{
			Name: check.Name, Deprecated: check.Deprecated,
			DeprecatedReason: check.DeprecatedReason, Method: g.buildMethodSchema(check.Method),
