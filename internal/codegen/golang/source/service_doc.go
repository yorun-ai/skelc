...
func goDocLines(name string, description string) []string {
	lines := codegen.SplitDocLines(description)
	if len(lines) == 0 {
		return nil
	}
...
func goMethodDocLines(name string, description string, example string, arguments []*MethodArgument, resultType *Type, outputDescription string, outputExample string) []string {
	docLines := goDocLines(name, codegen.MergeDescriptionAndExample(description, example))
	if len(docLines) == 0 {
	...
	outputLines := codegen.SplitDocLines(codegen.MergeDescriptionAndExample(outputDescription, outputExample))
